...
#' @title FrequencyTableMMM Generate Frequency Distribution Table for Fish Length Data Across Months
...
#' @param output_file Character string specifying the output Excel file name. Defaults to "FrequencyTableMM_Output.xlsx".
...
#' result <- FrequencyTableMM(data = sample_data, output_file = tempfile(fileext = ".xlsx"))
#'
#' # Create frequency table with custom bin width and specific date
#' result <- FrequencyTableMM(data = sample_data, bin_width = 2,
...
#' @export
#'
FrequencyTableMM <- function(data, bin_width = NULL, Lmax = NULL, output_file = "FrequencyTableMM_Output.xlsx",
  ...
  openxlsx::addWorksheet(wb, "FrequencyTableMM")
  openxlsx::writeData(wb, sheet = "FrequencyTableMM", x = result, rowNames = FALSE)
...
beledata <- read_excel("Bele.xlsx", sheet = 3)

bdata <- beledata %>% select(1:2)

FrequencyTableMM(data = bdata, date_config = list( day = 14, year = 2024 ))
